...
img_output = cv2.imread("testimage.png", 10)
...
for y in range(21):
  for x in range(21):
    ...
    average = temp.mean(axis=0).mean(axis=0)
    #white
    if between((average[0] + average[1] + average[2])/3,  whitepoint, 10):
        Matrix[y][x] = 0
    #black
    elif between((average[0] + average[1] + average[2])/3,  blackpoint, 10):
        Matrix[y][x] = 1
    #blue
    elif between(average[0], bluepoint, 10) and between((average[1] + average[2])/2, blackpoint, 10):
        Matrix[y][x] = 2
    #red
    elif between(average[1], redpoint, 10) and between((average[0] + average[2])/2, blackpoint, 10):
        Matrix[y][x] = 3
    #green
    elif between(average[2], greenpoint, 10) and between((average[1] + average[0])/2, blackpoint, 10):
        Matrix[y][x] = 4
